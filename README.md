...
- **网页上下文** — 可直接读取当前登录态页面的标题、正文、元信息、标题结构与可见链接
- **插件读取页面** — Claude/插件可通过本地只读接口按需读取当前浏览器页面信息
...
这会注册 Chrome Native Messaging Host。之后打开插件侧栏时，浏览器会自动在后台启动本地 bridge，不需要再手动运行 `pnpm dev:bridge`。

扩展已固定开发版 ID 为 `jnbmilnhcipmgeakjeoenjagfjpfpmbe`，安装脚本会将 Native Messaging Host 限定给这个扩展使用。
...
如果需要调试 bridge 本身，仍然可以手动运行 `pnpm dev:bridge` 查看日志；正常使用不需要打开终端常驻。
...
| 桥接服务地址 | WebSocket 连接地址，默认 `ws://127.0.0.1:18765` |

## 浏览器页面读取接口

桥接服务启动后，Claude Code 或本地插件可以读取当前活动标签页的页面信息：

```bash
curl -s "http://127.0.0.1:18765/browser/page-context?maxLength=12000&includeLinks=true"
```

返回 JSON 包含 `url`、`title`、`selectedText`、`bodyText`、`meta`、`headings` 和 `links`。接口只监听 `127.0.0.1`，并依赖已连接的浏览器扩展转发当前页面内容。
